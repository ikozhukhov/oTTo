...
def rremove(req, error):
    req.sock.delfid(req.ifcall.fid)
    return


def rclunk(req, error):
    return


def rwrite(req, error):
    return
...
class Server(object):
    ...
    def __init__(self, listen, authmode=None, fs=None, user=None, dom=None, key=None, chatty=False, dotu=False):
        if authmode is None or authmode == 'none':
            self.authfs = None
        elif authmode == 'pki':
            import pki
    ...
    def respond(self, req, error=None, errno=None):
        ...
        if hasattr(self, name):
            ...
            raise ServerError("can not handle message type %s" % cmdName[req.ifcall.type])
        ...
        try:
            ...
            if self.chatty:
                print >> sys.stderr, "socket error: %s" % e.args[1]
            self.shutdown(s)
        except EofError, e:
            if self.chatty:
                print >> sys.stderr, "socket closed: %s" % e.args[0]
            self.shutdown(s)
        except Exception, e:
            if self.chatty:
                print >> sys.stderr, "socket error: %s" % str(e.args)
            self.shutdown(s)
    ...
    def tread(self, req):
        ...
        if req.ifcall.offset < 0 or \
                ((req.fid.qid.type & QTDIR) and (req.ifcall.offset != 0) and (req.ifcall.offset != req.fid.diroffset)):
